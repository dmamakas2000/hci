...
public final class ConicalGradientPaint implements java.awt.Paint {
     ...
     */
    public ConicalGradientPaint( final java.awt.geom.Point2D CENTER, final float[] GIVEN_FRACTIONS,
            final java.awt.Color[] GIVEN_COLORS ) throws IllegalArgumentException {
        this( false, CENTER, 0.0f, GIVEN_FRACTIONS, GIVEN_COLORS );
    }

    /**
     * Enhanced constructor which takes the FRACTIONS in degress from 0.0f to 360.0f and also an GIVEN_OFFSET in degrees around the
     * rotation CENTER
     *
     * @param USE_DEGREES
     * @param CENTER
     ...
     * @throws IllegalArgumentException
     */
    public ConicalGradientPaint( final boolean USE_DEGREES, final java.awt.geom.Point2D CENTER, final float GIVEN_OFFSET,
            final float[] GIVEN_FRACTIONS, final java.awt.Color[] GIVEN_COLORS ) throws IllegalArgumentException {
        // Check that fractions and colors are of the same size
        if ( GIVEN_FRACTIONS.length != GIVEN_COLORS.length ) {
            throw new IllegalArgumentException( "Fractions and colors must be equal in size" );
        }

        final java.util.ArrayList<Float> FRACTION_LIST = new java.util.ArrayList<>( GIVEN_FRACTIONS.length );
        final float OFFSET;
        if ( USE_DEGREES ) {
        ...
        final java.util.List<java.awt.Color> COLOR_LIST = new java.util.ArrayList<>( GIVEN_COLORS.length );
        ...
        final java.util.SortedSet<Float> SORTED_FRACTIONS = new java.util.TreeSet<>( FRACTION_COLORS.keySet() );
        ...
        // Set the values
        this.CENTER = CENTER;
        this.COLORS = COLOR_LIST.toArray( new java.awt.Color[] {} );
        ...
        for ( int i = 0; i < MAX_FRACTIONS; i++ ) {
            this.FRACTION_ANGLES[i] = FRACTION_LIST.get( i ) * 360;
        }
        ...
        for ( int i = 0; i < ( this.COLORS.length - 1 ); i++ ) {
            this.RED_STEP_LOOKUP[i] = ( ( this.COLORS[i + 1].getRed() - this.COLORS[i].getRed() )
                    * ConicalGradientPaint.INT_TO_FLOAT_CONST ) / ( this.FRACTION_ANGLES[i + 1] - this.FRACTION_ANGLES[i] );
            this.GREEN_STEP_LOOKUP[i] = ( ( this.COLORS[i + 1].getGreen() - this.COLORS[i].getGreen() )
                    * ConicalGradientPaint.INT_TO_FLOAT_CONST ) / ( this.FRACTION_ANGLES[i + 1] - this.FRACTION_ANGLES[i] );
            this.BLUE_STEP_LOOKUP[i] = ( ( this.COLORS[i + 1].getBlue() - this.COLORS[i].getBlue() )
                    * ConicalGradientPaint.INT_TO_FLOAT_CONST ) / ( this.FRACTION_ANGLES[i + 1] - this.FRACTION_ANGLES[i] );
            this.ALPHA_STEP_LOOKUP[i] = ( ( this.COLORS[i + 1].getAlpha() - this.COLORS[i].getAlpha() )
                    * ConicalGradientPaint.INT_TO_FLOAT_CONST ) / ( this.FRACTION_ANGLES[i + 1] - this.FRACTION_ANGLES[i] );
        }
    }

    /**
     * Recalculates the fractions in the FRACTION_LIST and their associated colors in the COLOR_LIST with a given OFFSET. Because the
     * conical gradients always starts with 0 at the top and clockwise direction you could rotate the defined conical gradient from
     * -180 to 180 degrees which equals values from -0.5 to +0.5
     *
     * @param FRACTION_LIST
     * @param COLOR_LIST
     * @param OFFSET
     ...
     */
    private java.util.HashMap<Float,java.awt.Color> recalculate( final java.util.List<Float> FRACTION_LIST,
            final java.util.List<java.awt.Color> COLOR_LIST, final float OFFSET ) {
        // Recalculate the fractions and colors with the given offset
        final int MAX_FRACTIONS = FRACTION_LIST.size();
        final java.util.HashMap<Float,java.awt.Color> FRACTION_COLORS = new java.util.HashMap<>( MAX_FRACTIONS );
        for ( int i = 0; i < MAX_FRACTIONS; i++ ) {
            ...
            if ( TMP_FRACTION <= 0 ) {
                ...
                if ( NEXT_FRACTION > 0 ) {
                    final java.awt.Color NEW_FRACTION_COLOR = getColorFromFraction( TMP_COLOR, NEXT_COLOR,
                            ( int ) ( ( NEXT_FRACTION - TMP_FRACTION ) * 10000 ), ( int ) ( ( -TMP_FRACTION ) * 10000 ) );
                ...
                if ( PREVIOUS_FRACTION < 1 ) {
                    final java.awt.Color NEW_FRACTION_COLOR = getColorFromFraction( TMP_COLOR, PREVIOUS_COLOR,
                            ( int ) ( ( TMP_FRACTION - PREVIOUS_FRACTION ) * 10000 ), ( int ) ( TMP_FRACTION - 1.0f ) * 10000 );
                    FRACTION_COLORS.put( 1.0f, NEW_FRACTION_COLOR );
                    FRACTION_COLORS.put( 0.0f, NEW_FRACTION_COLOR );
                }
            }
    ...
    /**
     * With the START_COLOR at the beginning and the DESTINATION_COLOR at the end of the given RANGE the method will calculate and
     * return the color that equals the given VALUE. e.g. a START_COLOR of BLACK (R:0, G:0, B:0, A:255) and a DESTINATION_COLOR of
     * WHITE(R:255, G:255, B:255, A:255) with a given RANGE of 100 and a given VALUE of 50 will return the color that is exactly in the
     * middle of the gradient between black and white which is gray(R:128, G:128, B:128, A:255) So this method is really useful to
     ...
     */
    public java.awt.Color getColorFromFraction( final java.awt.Color START_COLOR, final java.awt.Color DESTINATION_COLOR,
            final int RANGE, final int VALUE ) {
        final float SOURCE_RED = START_COLOR.getRed() * ConicalGradientPaint.INT_TO_FLOAT_CONST;
        final float SOURCE_GREEN = START_COLOR.getGreen() * ConicalGradientPaint.INT_TO_FLOAT_CONST;
        final float SOURCE_BLUE = START_COLOR.getBlue() * ConicalGradientPaint.INT_TO_FLOAT_CONST;
        final float SOURCE_ALPHA = START_COLOR.getAlpha() * ConicalGradientPaint.INT_TO_FLOAT_CONST;

        final float DESTINATION_RED = DESTINATION_COLOR.getRed() * ConicalGradientPaint.INT_TO_FLOAT_CONST;
        final float DESTINATION_GREEN = DESTINATION_COLOR.getGreen() * ConicalGradientPaint.INT_TO_FLOAT_CONST;
        final float DESTINATION_BLUE = DESTINATION_COLOR.getBlue() * ConicalGradientPaint.INT_TO_FLOAT_CONST;
        final float DESTINATION_ALPHA = DESTINATION_COLOR.getAlpha() * ConicalGradientPaint.INT_TO_FLOAT_CONST;
        ...
        return new java.awt.Color( SOURCE_RED + RED_FRACTION * VALUE, SOURCE_GREEN + GREEN_FRACTION * VALUE,
                SOURCE_BLUE + BLUE_FRACTION * VALUE, SOURCE_ALPHA + ALPHA_FRACTION * VALUE );
    }

    @Override
    public java.awt.PaintContext createContext( final java.awt.image.ColorModel COLOR_MODEL, final java.awt.Rectangle DEVICE_BOUNDS,
            final java.awt.geom.Rectangle2D USER_BOUNDS, final java.awt.geom.AffineTransform TRANSFORM,
            final java.awt.RenderingHints HINTS ) {
        final java.awt.geom.Point2D TRANSFORMED_CENTER = TRANSFORM.transform( this.CENTER, null );
    ...
    private final class ConicalGradientPaintContext implements java.awt.PaintContext {
        ...
        public java.awt.image.Raster getRaster( final int X, final int Y, final int TILE_WIDTH, final int TILE_HEIGHT ) {
            final double ROTATION_CENTER_X = -X + this.CENTER.getX();
            final double ROTATION_CENTER_Y = -Y + this.CENTER.getY();

            final int MAX = ConicalGradientPaint.this.FRACTION_ANGLES.length;
            ...
            for ( int py = 0; py < TILE_HEIGHT; py++ ) {
                for ( int px = 0; px < TILE_WIDTH; px++ ) {
                    ...
                    for ( int i = 0; i < ( MAX - 1 ); i++ ) {
                        if ( ( angle >= ConicalGradientPaint.this.FRACTION_ANGLES[i] ) ) {
                            currentRed = ConicalGradientPaint.this.COLORS[i].getRed() * ConicalGradientPaint.INT_TO_FLOAT_CONST
                                    + ( angle - ConicalGradientPaint.this.FRACTION_ANGLES[i] )
                                            * ConicalGradientPaint.this.RED_STEP_LOOKUP[i];
                            currentGreen = ConicalGradientPaint.this.COLORS[i].getGreen() * ConicalGradientPaint.INT_TO_FLOAT_CONST
                                    + ( angle - ConicalGradientPaint.this.FRACTION_ANGLES[i] )
                                            * ConicalGradientPaint.this.GREEN_STEP_LOOKUP[i];
                            currentBlue = ConicalGradientPaint.this.COLORS[i].getBlue() * ConicalGradientPaint.INT_TO_FLOAT_CONST
                                    + ( angle - ConicalGradientPaint.this.FRACTION_ANGLES[i] )
                                            * ConicalGradientPaint.this.BLUE_STEP_LOOKUP[i];
                            currentAlpha = ConicalGradientPaint.this.COLORS[i].getAlpha() * ConicalGradientPaint.INT_TO_FLOAT_CONST
                                    + ( angle - ConicalGradientPaint.this.FRACTION_ANGLES[i] )
                                            * ConicalGradientPaint.this.ALPHA_STEP_LOOKUP[i];
                            continue;
                        }
                    }
